...
async function handler(req, res) {
	...
	if (!token) {
		return sendJsonResponse(res, 401, { success: false, message: 'Không có quyền truy cập' });
	}
	let userId;
	try {
	...
	const userAct = await userActivityRepository.getByUserAndActivity(userId, activityId);
	if (!userAct) {
		return sendJsonResponse(res, 409, { success: false, message: 'Bạn chưa tham gia hoạt động này' });
	}

	await userActivityRepository.delete(userAct._id);
	await activityRepository.update(activityId, { $inc: { curAmountOfPerson: -1 } });

	return sendJsonResponse(res, 200, { success: true, message: 'Rời khỏi hoạt động thành công' });
}
