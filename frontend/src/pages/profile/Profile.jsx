...
export default function Profile() {
	...
	return (
		<div className={styles.Profile}>
			<div className={`${styles.logoutOverlay} ${logoutLoading ? styles.active : ''}`}>Đang đăng xuất...</div>
				...
				<div className={styles.leftCol}>
					...
					<div className={styles.infoCard}>
						...
						<div className={styles.row}>
							...
							<span className={styles.value}>{user?.birthDate ? new Date(user.birthDate).toLocaleDateString('vi-VN') : 'Chưa cập nhật'}</span>
						</div>
						<div className={styles.row}>
						...
						<button className={styles.logoutBtn} onClick={handleLogout} disabled={logoutLoading}>
					...
					{rankData.map((user, index) => (
						<li key={user.id} className={styles.rankItem}>
							...
							<span className={styles.name}>
								{user.name}
								{index === 0 && <Crown className={styles.crown} size={18} />}
							</span>
