			...
			<div className={styles.container}>
				...
				<div className={styles.grid}>
					{featuredActivities.map((activity, index) => (
						<Card key={activity.id} className={`${styles.card} fade-in`} style={{ animationDelay: `${index * 0.2}s` }}>
							<div className={styles.imageWrapper}>
								<img src={activity.image} alt={activity.name || 'Hình ảnh hoạt động'} className={styles.image} loading='lazy' />
							...
							<CardContent className={styles.cardContent}>
								...
								<div className={styles.btnWrapper}>
									<Link to={`/activities/${activity.id}`} className={styles.button} aria-label={`Xem chi tiết hoạt động ${activity.name}`}>
										Xem chi tiết
									</Link>
								</div>
							</CardContent>
				...
				<div className={styles.footer}>
					<Link to={ROUTES.ACTIVE.path} className={styles.buttonOutline} aria-label='Xem tất cả hoạt động'>
